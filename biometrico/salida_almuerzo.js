...
runFlow(async ({ page, takeShot, addStep }) => {
    ...
    await page.locator('#markingtype').selectOption('11');
    ...
    await page.waitForLoadState('networkidle');
    await takeShot(page, '05-reporte-asistencia', 'Pantalla de reporte asistencia general');

    await page.getByRole('textbox', { name: 'Fecha Desde' }).click();
    const today = new Date().getDate().toString();
    await page.getByRole('link', { name: today, exact: true }).click();
    await takeShot(page, '06-fecha-desde', 'Fecha desde seleccionada');

    await page.getByRole('textbox', { name: 'Fecha Hasta' }).click();
    await page.getByRole('link', { name: today, exact: true }).click();
    await takeShot(page, '07-fecha-hasta', 'Fecha hasta seleccionada');

    await page.locator('#province').selectOption('17');
    await page.locator('#city').selectOption('1701');
    await takeShot(page, '08-filtros-finales', 'Filtros finales configurados');

    await page.getByRole('button', { name: 'Ver Reporte' }).click();
    await page.waitForLoadState('networkidle');
    await takeShot(page, '09-reporte-generado', 'Reporte generado correctamente');

    await page.getByRole('link', { name: 'Salir' }).click();
    await takeShot(page, '10-salida', 'Salida del sistema realizada');
});